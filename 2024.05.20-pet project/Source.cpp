...
class Bank {
	int code = 0;
	svector<Account> acclist = svector <Account>();
...
public:
	...
	Bank(Bank&& bank) noexcept
	{
		...
		bank.code = 0;
		bank.acclist.~svector();
	}


	~Bank()
	{
		this->acclist.~svector();
		code = 0;
	}
	...
	{
		this->acclist.removeelement(acc);
	}

	bool exists(const Account& account) const
	{
		for(int i = 0; i < this->acclist.getsize(); i++)
		{
			if (this->acclist[i].getnum() == account.getnum())
			{
				return true;
			}
		}
	...
	void setaccv(svector <Account> a)
	{
		this->acclist.~svector();
		this->acclist = (a);
	}
...
int main()
{
	...
	while (true)
	{
		...
		else if (n == 7)
		{
			...
			if (k > 0 && u > 0)
			{
				Account aacc = r.search(a);
				int inda = v.indexof(aacc);
				Account bacc = r.search(b);
